...
void PMLogic::Score::StartTimedDecrease() {
    ...
    PMLogic::Helper::TimeManager::GetInstance().lock()->AddTimer(scoreTimer);
    stopWatch->Start();
}

PMLogic::Score::Score() : stopWatch(std::make_shared<Helper::StopWatch>()), currentScore(0) {
    StartTimedDecrease();
    PMLogic::Helper::TimeManager::GetInstance().lock()->AddStopWatch(stopWatch);
}
...
int PMLogic::Score::GetScore() const { return currentScore; }

void PMLogic::Score::IncreaseScore(const int& amount) { currentScore += amount; }
