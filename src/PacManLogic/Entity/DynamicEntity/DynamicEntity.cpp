...
DynamicEntity::DynamicEntity(const Coordinate2D::NormalizedCoordinate& startPosition,
                             const Coordinate2D::Coordinate& size, const float& defaultSpeed)
    : PMLogic::Entity(startPosition, size), spawn(startPosition), currentDirection(Coordinate2D::Direction_Right),
      nextDirection(currentDirection), viableDirections(Coordinate2D::directions2D), defaultSpeed(defaultSpeed),
      speed(defaultSpeed), isKillable(true),
...
Coordinate2D::NormalizedCoordinate DynamicEntity::GetNextPosition(
    const Coordinate2D::DiscreteDirection2D& direction) const {
    const float deltaTime{PMLogic::Helper::TimeManager::GetInstance().lock()->GetDeltaTime()};
...
Coordinate2D::NormalizedCoordinate DynamicEntity::GetNextPosition() const { return GetNextPosition(GetDirection()); }
...
float DynamicEntity::GetSpeed() const { return speed; }

void DynamicEntity::SetSpeed(const float& newSpeed) { speed = newSpeed; }
...
Coordinate2D::DiscreteDirection2D DynamicEntity::GetDirection() const { return currentDirection; }

bool DynamicEntity::GetIsKillable() const { return isKillable; }

void DynamicEntity::SetIsKillable(const bool& newIsKillable) { isKillable = newIsKillable; }
...
bool DynamicEntity::WillCollide(const PMLogic::Entity& entity,
                                const Coordinate2D::DiscreteDirection2D& direction) const {
    return Coordinate2D::IsOverlapping(GetNextPosition(direction), GetSize(), entity.GetPosition(), entity.GetSize());
}

float DynamicEntity::GetDefaultSpeed() const { return defaultSpeed; }

Coordinate2D::NormalizedCoordinate DynamicEntity::GetSpawn() const { return spawn; }
