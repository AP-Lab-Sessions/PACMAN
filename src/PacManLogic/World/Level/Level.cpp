...
bool Level::CharIsPath(const char& c) const { return c != '#' && c != '\n'; }
...
void Level::Load() {
    ...
    Coordinate2D::NormalizedCoordinate currentPos{Coordinate2D::normalizedMin + (size.GetX() / 2.0f),
                                                  Coordinate2D::normalizedMin};
    ...
    for (const char& currentChar : levelStr) {
        ...
        case '.': {
            ...
            const Coordinate2D::NormalizedCoordinate centeredPosition{
...
std::weak_ptr<PacMan> Level::GetPlayer() const { return player; }
...
void Level::Update(const EntityCollectedEvent&) { collectablesCount--; }
